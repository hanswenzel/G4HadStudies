...
TGraph * cx;
TGraph * kplus;
std::string names[8] = {"pi+", "pi0", "p", "n", "gamma", "K+", "meson", "fragment"};
TGraph * particles[8];
...
void total_XS( std::string filename, std::string material_name )
{
  ...
  for( int i = 0; i < n_entries; ++i ) {
    ...
    if( intType[0] == 2 ) n_elastic++;
    else {
      n_inelastic++;

      for( int j = 0; j < npart; ++j ) {
        if( pdg[j] == 111 ) n_cx++;
        if( pdg[j] == 321 ) n_kplus++;
        
        if( pdg[j] == 211 ) counts[0]++;
        else if( pdg[j] == 111 ) counts[1]++;
        else if( pdg[j] == 2212 ) counts[2]++;
        else if( pdg[j] == 2112 ) counts[3]++;
        else if( pdg[j] == 22 ) counts[4]++;
        else if( pdg[j] == 321 ) counts[5]++;
        else if( pdg[j] < 9999 ) counts[6]++;
        else counts[7]++;
  ...
  for( int i = 0; i < 8; ++i ) counts[i] /= n_inelastic;
  ...
  printf( "Kinetic energy = %2.0f Total XS = %2.1f inel = %2.1f elastic = %2.1f CX = %2.1f from file inel %2.1f elastic %2.1f kplus %2.1f\n", enerPrimGen, total_XS, inelastic_XS, elastic_XS, cx_XS, inelasticXS, elasticXS, kplus_XS );

  int n = total->GetN();
  total->SetPoint( n, enerPrimGen, total_XS );
  elastic->SetPoint( n, enerPrimGen, elastic_XS );
  inelastic->SetPoint( n, enerPrimGen, inelastic_XS-cx_XS-kplus_XS );
  cx->SetPoint( n, enerPrimGen, cx_XS );
  kplus->SetPoint( n, enerPrimGen, kplus_XS );
...
void pion_xs()
{
  ...
  int colors[8] = { kBlack, kRed, kBlue, kGreen+2, kMagenta+3, kOrange+1, kCyan+2, kRed+3 };
  ...
  total = new TGraph();
  total->SetName("total");
  total->SetTitle(";Pion KE (MeV);XS (mb)");
  elastic = new TGraph();
  elastic->SetName("elastic");
  elastic->SetTitle(";Pion KE (MeV);XS (mb)");
  inelastic = new TGraph();
  inelastic->SetName("inelastic");
  inelastic->SetTitle(";Pion KE (MeV);XS (mb)");
  cx = new TGraph();
  cx->SetName("cx");
  cx->SetTitle(";Pion KE (MeV);XS (mb)");
  kplus = new TGraph();
  kplus->SetName("kplus");
  kplus->SetTitle(";Pion KE (MeV);XS (mb)");
  ...
  particles[0]->SetTitle(";Pion kinetic energy (MeV);Particles per inelastic interaction");
  ...
  TFile * fout = new TFile( "pion_xs.root", "RECREATE" );
  fout->cd();
  total->Write();
  inelastic->Write();
  elastic->Write();
